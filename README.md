...
┌─────────────────────────────────────────────────────────────┐
│                    Hercules 3.07 (Linux host)                │
│  ┌───────────────────────────────────────────────────────┐  │
│  │              MVS 3.8j — Distribution TK5R              │  │
│  │                                                         │  │
│  │  ┌──────────┐   ┌──────────┐   ┌──────────────────┐    │  │
│  │  │  TSO     │   │  JES2    │   │   Spool (DASD)    │    │  │
│  │  │ HERC01   │──▶│ (batch   │──▶│  dasd/spool0.249  │    │  │
│  │  │ (ISPF)   │   │  queue)  │   │                    │    │  │
│  │  └──────────┘   └──────────┘   └──────────────────┘    │  │
│  │                                                         │  │
...
│  └───────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
         3270 Terminal (c3270) ── TCP access on :3270
JCL execution flow (3 steps)
COBCOMP  ──▶  LKED  ──▶  RUNBANK
(IKFCBL00)   (HEWL)     (ACCTMGMT)
   │            │            │
   ▼            ▼            ▼
&&LOADSET   LOAD(ACCTMGMT) SYSOUT (menu + results)
...
Record	Field	PIC	Description
ACCOUNT-REG	ACCOUNT-ID	9(10)	Account identifier
	BALANCE	9(7)V99 COMP-3	Current balance (packed decimal)
	ACCOUNT-STATUS	X(10)	Account status (ACTIVE/...)
CUSTOMER-REG	CUSTOMER-ID	9(10)	Customer identifier
	FIRST-NAME	X(20)	First name
	LAST-NAME	X(20)	Last name
TRANSACTIONS-REG	TRANSACTION-ID	9(10)	Transaction sequence number
	ACCOUNT-ID	9(10)	Affected account
	AMOUNT	9(7)V99 COMP-3	Transaction amount
	TRANSACTION-DATE	9(8)	Date (CCYYMMDD)
	TRANSACTION-TYPE	X(10)	DEPOSIT / WITHDRAWAL / REJECTED
...
Cause: the line volume from the previous incident filled the spool disk to 100%. TSO/ISPF could no longer register I/O.
Failed attempt: purge commands (/$p outall, /$p A,ALL) failed with INVALID OPERAND due to limitations of the JES2 release in TK5.
...
Symptom on the next IPL:
  $HASP486 SPOOL0 DAMAGED CHECKPOINT DATA SET DETECTED -- REPLY Y OR N TO CONTINUE
  $HASP479 UNABLE TO OBTAIN CKPT DATA SET LOCK -- REPLY Y OR N TO CONTINUE
Fix: formal reply to the WTOR from the Hercules console with the / prefix (required to direct the command to the MVS guest, not to Hercules itself):
  /r 0,y
  /r 1,y

After the JES2 warm-start, it was verified that no orphaned jobs remained, using /$d a,all → NO ACTIVE JOBS.
...
Final result
COBCOMP  RC=0004  (warning IKF4072I-W, non-blocking — see section 7)
LKED     RC=0000
RUNBANK  RC=0000

Business validation (starting balance 500.00):

Deposit +100.00 → 600.00
Withdrawal -500.00 → 100.00
Final balance inquiry → CURRENT BALANCE: 000010000 = $100.00 ✔
...
The schema below is a reference design documenting how persistence would be structured if the program were migrated to DB2, keeping a 1:1 mapping with the COBOL records already defined. It serves as a specification for a future migration (real DB2 for z/OS, or even a modern relational engine for local testing purposes).
...
CREATE TABLE BANKADM.ACCOUNT (
    ...
    CONSTRAINT FK_ACCOUNT_CUSTOMER FOREIGN KEY (CUSTOMER_ID)
        REFERENCES BANKADM.CUSTOMER (CUSTOMER_ID)
        ON DELETE RESTRICT,
...
CREATE TABLE BANKADM.TRANSACTIONS (
    ...
    CONSTRAINT FK_TXN_ACCOUNT FOREIGN KEY (ACCOUNT_ID)
        REFERENCES BANKADM.ACCOUNT (ACCOUNT_ID)
        ON DELETE RESTRICT,
    CONSTRAINT CK_TXN_TYPE CHECK
        (TXN_TYPE IN ('DEPOSIT', 'WITHDRAWAL', 'REJECTED')),
...
6.3 COBOL ↔ DB2 mapping (for a future EXEC SQL migration)
COBOL field (WORKING-STORAGE)	DB2 column	Comment
ACCOUNT-ID (9(10))	ACCOUNT.ACCOUNT_ID (DECIMAL(10,0))	Primary key
BALANCE (9(7)V99 COMP-3)	ACCOUNT.BALANCE (DECIMAL(9,2))	COBOL packed decimal maps directly to SQL DECIMAL
ACCOUNT-STATUS (X(10))	ACCOUNT.ACCOUNT_STATUS (CHAR(10))	Constrained with CHECK instead of validated only inside the program
...
TRANSACTION-DATE (9(8) CCYYMMDD)	TRANSACTIONS.TXN_DATE (DATE)	Requires conversion from 9(8) to a native DATE type
...
CUSTOMER-REG not integrated into the business logic.
